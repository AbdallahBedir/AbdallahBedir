...
const MyTextField = ({ label, ...props }) => {
    ...
    return (
        <WhiteTextField error={meta.touched && meta.error ? true : false} variant="filled" label={label} fullWidth size="small"  
            helperText={<ErrorMessage name={props.name}/>} {...field} {...props}/>
    );
  };
                            ...
                            {formik => (
                                <div>
                                    <Form>              
                                        ...
                                        <MyTextField name="message" type="text" label="Message" multiline rows={5}/>
    
                                        <Button type="submit" className={classes.submit} disabled={formik.isSubmitting} variant="contained" color="secondary">Send Message</Button>
                                        {formik.isSubmitting ? <CircularProgress className={classes.spinner} size={24} thickness={4} /> : ''}
                                    </Form>
                                    <Snackbar open={toastState.isOpen} autoHideDuration={3000} 
                                    onClose={handleToastClose} TransitionComponent={Grow}>
                                        <MuiAlert elevation={6} variant="filled" onClose={handleToastClose} severity={toastState.success ? 'success' : 'error'}>
                                            {toastState.success ? 'Message sent successfully' : 'An error occured while sending the message'}
                                        </MuiAlert>
                                    </Snackbar>
                                </div>
                            )}
